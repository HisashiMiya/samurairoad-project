(() => {
  ...
  function isPanWhileDrawingEnabled() {
    return !!(ui.chkPan()?.checked || ui.chkPanBar()?.checked);
  }

  function syncPanCheckboxes(source) {
    const checked = !!source?.checked;
    if (ui.chkPan() && ui.chkPan() !== source) ui.chkPan().checked = checked;
    if (ui.chkPanBar() && ui.chkPanBar() !== source) ui.chkPanBar().checked = checked;
    updateDrawBarTexts();
    updateMapCursor();
  }
  ...
  function updateDrawBarTexts() {
    ...
    if (title) {
      title.textContent = drawing ? (panMode ? '🖐️ 地図移動中' : '✏️ 描画中') : '✏️ ルート作成';
    }
    if (hint) {
      if (drawing) {
        hint.textContent = panMode
          ? '地図移動ONです。地図の位置をずらせます。線は描かれません。'
          : '地図上をドラッグすると線が引かれます。地図を動かしたいときは下のチェックをONにしてください。';
      } else {
  ...
  function updateMapCursor() {
    const container = state.container || state.map?.getContainer?.();
    if (!container) return;
    ...
    container.style.cursor = isPanWhileDrawingEnabled() ? 'grab' : 'crosshair';
  }
  ...
  function start() {
    ...
    updateDrawBarTexts();
    updateMapCursor();
    syncPanCheckboxes(isPanWhileDrawingEnabled() ? (ui.chkPanBar()?.checked ? ui.chkPanBar() : ui.chkPan()) : (ui.chkPanBar()?.checked ? ui.chkPanBar() : ui.chkPan()));
    toast(isPanWhileDrawingEnabled() ? '地図移動モードです。地図の位置をずらせます。線は描かれません。' : safeT('re_msg_draw', '描画モード：地図上をドラッグして線を引いてください'));
  }
  ...
  function wireUI() {
    ...
    ui.chkPan()?.addEventListener('change', (e) => syncPanCheckboxes(e.target));
    ui.chkPanBar()?.addEventListener('change', (e) => syncPanCheckboxes(e.target));
